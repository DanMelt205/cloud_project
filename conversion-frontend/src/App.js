...
const Header = () => {
  return (
    <div className="text-center flex-this">
      ...
      This product is grabbing from out openly avaiable api, just contact the email below for more information.
      Happy converting!
    </p>
    </div>
  );
...
const AllConversions = p => {
  let keys = Object.keys(p.pass);
  return (
    <>
      <div className="card col-4" style={{ width: 150 }}>
        <div className="card-body">
          <h5 className="card-title">Past Conversions</h5>
          <h6>From:&nbsp; {p.pass[keys[1]]} {keys[1]} </h6>
          <h6>To:&nbsp; {p.pass[keys[0]]} {keys[0]} </h6>
        </div>
      </div>
    </>
  );
        ...
        setPassConversions(response.data.conversions.map((pass, i) => {
          return (
            <>
              ...
              <AllConversions className="row" pass={pass} i={i} />
